...
'''
read_csv(): CSV 파일을 데이터프레임으로 변환한다. 별도의 옵션이 없으면
            첫번째행을 컬럼으로 간주한다. 그리고 행은 정수형으로 지정된다.
'''
...
'''
header옵션 : 열의 이름으로 사용될 행의 번호를 지정한다. 기본값은 0.
...
'''
df5 = pd.read_csv(file_path, names=['삼장법사','손오공','저팔게','사오정'])
print(df5)
...
끝에서 부터 맞춰 지정된다. 즉 c0는 인덱스가 되고, c1~c3까지
데이터가 된다.
'''
