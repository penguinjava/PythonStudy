...
'''
데이터프레임에서 특정 원소 하나를 선택할 때는 Java의 배열과
유사하게 행과 열의 인덱스를 통해 접근할 수 있다.
'''
#딕셔너리를 데이터프레임으로 변환
...
'''
현재는 숫자형 인덱스만 부여된 상태이므로 '유비'를 통해서는
데이터에 접근할 수 없다.
'''
#a = df.loc['유비','수학'] -오류발생-
...
'''
이름 컬럼을 데이터프레임의 행 인덱스로 지정한다. 이 경우 출력시
숫자형 인덱스는 제거된다.
'''
...
#콜론을 통해 범위를 지정하여 범위를 통해 2개 이사으이 행과 열을 선택
...
#2개 이사으이 행과 열을 선택하여 요소를 지정
print(f"{'개별 선택 출력':-^30}")
g = df.loc[['유비','관우'],['영어','체육']]
print(g)
h= df.iloc[[0,1],[1,3]]
print(h)
# 범위를 통해 2개 이사으이 행과 열을 선택
print(f"{'범위 지정 출력':-^30}")
i = df.loc['유비':'관우','영어':'체육']
print(i)
#행은 0~1까지만 선택하고, 열은 1~마지막까지 선택한다.
j= df.iloc[0:2,1:]
print(j)