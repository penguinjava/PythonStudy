...
#3행 5열인 데이터프레임으로 변환. 이때 인덱스를 지정함.
...
print(new_df2)
'''
NaN : Not a Number의 이니셜로 지역하면 숫자가 아니다라는 뜻.
    유효한 값이 존재하지 않는 누락데이터를 의미한다.
'''

#완전히 새로운 인덱스로 제지정하면 모든 데이터가 NaN으로 초기화된다.
...
#행 인덱스를 정수형 인덱스로초기화한다. 이때 기존 행 인덱스는
#컬럼으로 이동한다.
...
#삭제된 c0 컬럼이 복원되면서 숫자형 인데긋로 변경된다.
