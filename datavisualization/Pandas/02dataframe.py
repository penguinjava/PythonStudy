...
'''
데이터 프레임(DataFrame)
: 1차원 배열 형태는 시리즈가 여러개 모여있는 형태로 2차원배열과 동일하다.
R의 데이터프레임을 착안하여 만들어지게 되었다.
행(Row)과 열(Column)로 구성된다.
'''

'''
딕셔너리의 Key는 컬럼명이 되고, Value는 행으로 변환된다. 즉 3행 5열인
데이터 프레임이 생성된다.
'''
...
#문자형 인덱스를 부여하더랃 정수형 인덱스는 사용할 수 있다.
stu2 = df.iloc[1]
print('stu1과 stu2 \n',stu1, '\n',stu2)
...
축 옵션
    ...
    열 삭제 : axis=1
여러개를 동시에 삭제할 때는 리스트 형태로 입력하면된다.
'''
...
'''
#첫번째 실행에는 inplace옵션이 없으므로 원본 객체에는 적용되지 않는다.
