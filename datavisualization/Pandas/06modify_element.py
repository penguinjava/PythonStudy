...
#숫자형 인덱스를 '이름'컬럼으로 변경한 후 원본에 저장
print(f"{'최초 출력':-^30}")
df.set_index('이름',inplace=True)
print(df)

print(f"{'이름을 index로 지정 후 출력':-^30}")
#0행 전체를 선택 후 하나의 요소를 지정하게 되므로 이와 같은
#형태로 사용하지 않기를 권고함.
#df.iloc[0][3] = 77 -경고 발생-
#이과 같이 사용. 컴마를 통해 하나의 요소를 직접 지정
df.iloc[0,3] = 77
print(df)
...
'''
기존에 지정된 인덱스를 '체육' 컬럼으로 대체한다.
즉 '이름'으로 지정되었던 인덱스는 삭제된다.
'''
print(new_df1)
...
#컬럼에서는 '국거'만 남게됨
print(new_df2)
#인덱스가 2개이므로 요소를 지정할대도 아래와 같이 작성해야한다.
