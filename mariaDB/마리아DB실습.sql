...
Ctrl + Shift + F9 : 현재 쿼리를 실행한다. 단 마지막에
	기술한 세미콜론 안으로 커서를 옮긴 후 실행해야한다.
*/

/*
테이블 생성하기
제약조건
	PRIMARY KEY : 기본키 지정. 해당 키로 지정된 컬럼은
		중복되지 않는 값이 입력되어야 한다.
		사람으로 따지면 주민번호와 동일하다.
	AUTO_INCREMENT : 자동증가 컬럼으로 지정한다. 1씩
		증가하는 순차적인 정수값이 자동으로 입력된다.
	UNSIGNED : 정수형 컬럼으로 지정하는 경우 음수는 사용하지
		않고, 양수의 범위만 사용한다. 이때 양의 범위가 2개로
		늘어난다.
*/
...
#데이터 입력하기
#혀식1 : 일련번호 컬럼은 주로 INSERT문에서 제외하는것이 좋다.
...
#			기술해야 하므로 실행시 오류가 발생할 수 있으므로 권장하지 않는다.
INSERT INTO tb_int
...
#2. 날짜형으로 구성된 테이블
/*
CURRENT_TIMESTAMP : 날짜형식으로 지정된 컬럼에 디폴트값으오
현재시각을 입력할 때 사용한다. 오라클의 sysdate와 동일하다.
NOW() : 날짜형식으로 지정된 컬럼에 현재시각을 입력한다. 초단위까지의
		시간이 입력된다.
*/
...
INSERT INTO tb_date (DATE1, DATE2) VALUES('2020-1-2',NOW());
INSERT INTO tb_date (DATE1) VALUES ('2024-1-22');
SELECT * FROM tb_date;
