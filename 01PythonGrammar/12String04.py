...
'''
문자 바꾸기
    : maketrans('바꿀문자','새문자')로 변환테이블을 만든 후
    translate()로 적용한다.
    아래 테이블은 a는 1, e는 2로 변경된다.
'''

print(f"{'maketrns()/translate()':-^30}")
str1 = "apple"
...
'''
문자열 상세 검사
    :문자열에서 숫자, 알파벳, 한글 있는 경우 True,
    그외 문자가 포함되어 있다면 False를 반환한다.
'''

print(f"{'isalnum()':-^30}")
str = 'python312좋아' # True
print(str.isalnum())
str = 'python3.12좋아^^'
print(str.isalnum()) # False

'''
시나리오] 입력한 문자열에 영문대문자, 소문자, 숫자만 포함되어
있다면 True, 나머지 문자가 하나라도 포함되면 False를 반환하는
프로그램을 작서하시오.
'''
