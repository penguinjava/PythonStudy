...
Built-in Function(내장함수)
    : 내장함수는 외부 모듈과 달리 import가 필요하지 않기 때문에
...
'''
순서가 있는 자료형(리스트, 튜플, 문자열 등)을 입력받아 인덱스값을
포함하는 객체를 반환한다.
'''
print(f"{'기본 내장 함수':-^30}")
# 리스트를 인자로 받은 후 인덱스와 함께 출려
...
'''
값을 차례대로 꺼낼 수 있는 객체.
for i in rage(100): 과 같이 작성하면 파이썬은 0부터
99까지의 값을 차례대로 꺼낼 수 있는 이터레이터를 생성하여
숫자를 생성하게된다.
...
while it:
    ...
    print(row, end=", ")
'''
위 문장은 더 이상 출력할 항목이 없을 경우 next()에서 예외가 발생되면서
실행이 중지된다. 이부분은 예외처리에서 학습할 예정이다.
'''
print()
